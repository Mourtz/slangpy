...
from sgl import DataType, Device, ResourceUsage, TypeReflection
import torch
...
def get_or_create_storage(context: CallContext, element_count: int, struct_size: int) -> Buffer:
    ...
    if len(GLOBAL_STORAGE[value]) == 0:
        return context.device.create_buffer(size=element_count*struct_size, struct_size=struct_size, usage=ResourceUsage.shared | ResourceUsage.unordered_access | ResourceUsage.shader_resource)
